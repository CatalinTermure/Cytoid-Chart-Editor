...
public class DragChildNoteController : NoteController
{
    ...
    public override void Initialize(Note note)
    {
        ...
        if(NextID > 0)
        {
            ...
            DragConnector.GetComponent<SpriteRenderer>().size = new Vector2(0.35f / gameObject.transform.localScale.x, GlobalState.GetDistance(x2, y2, x1, y1) / gameObject.transform.localScale.x);
